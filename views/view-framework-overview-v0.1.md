...
**Status:** **Pre-pilot-review public draft, §1 and §2 only.** §§3–5 and the reference apparatus are held until external-reader feedback on §1 + §2 has been received and folded (per the artefact-category specification's first-instance pilot-reader requirement). Feedback is sought through an open call: see the [open-call brief](../docs/view-framework-overview-v0.1-pilot-reader-brief.md) for the four feedback questions and the independence criteria. Readers outside the Freiburg–NIST orbit, and with no prior familiarity with this project's vocabulary, are the target audience for this review.
