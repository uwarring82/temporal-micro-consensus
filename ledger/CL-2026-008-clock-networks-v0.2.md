...
The Ledger evaluates **C2** against Coastline v0.4. C1 is a feasibility statement reviewable on its own merits.
...
When this entry's v0.1 was written, the Coastline cited only special relativity (proper-time relations among inertial worldlines) and an explicit GR-regime boundary statement was a deferred candidate — making this the entry where that deferral became load-bearing. **Coastline v0.4 has since written the boundary:** its *Regime of Validity* extends the framework to the SR-and-weak-field / post-Newtonian regime — accelerated worldlines and worldlines at differing gravitational potential, curvature entering perturbatively — under a comparison-protocol proviso. The two proposals fall within that now-explicit scope:

- Covey's signal `Δω ∝ d²/R³` is a **spacetime-curvature (tidal)** effect — a *higher-order* term in the post-Newtonian expansion, **still within weak-field GR**, but **outside the Coastline's currently written scope**, which cites SR proper-time relations among inertial worldlines (External Constraints item 2). The issue is the framework's stated boundary, not a departure from weak-field physics.
...
With v0.4's *Regime of Validity* in place, the framework **can** now address C2's regime: the curvature residual Δω is the incompressible cross-probe part (Claim II.3 / Claim IV) within the weak-field scope, under the comparison-protocol proviso that distributed clock networks satisfy through their master-clock / fiber-link reference. **The framework-side gap that v0.1 flagged (D1) is therefore closed;** what remains is the experimental-witness gap (D2).

### Verdict on the structural claim

At the level of the formalism, C2's structural content is **framework-legible and, for the cross-probe anchor, framework-compatible**: `Δω` is a textbook Claim IV cross-probe mismatch, and the leading-consensus-cancels / curvature-residual-survives structure operationalises Claim II.3 compressibility cleanly. With the GR boundary now written (Coastline v0.4), the **sole** remaining condition blocking a COMPATIBLE classification is the **experimental-witness** gap shared with CL-2026-006/007 — the GR signature must be discriminable from technical decoherence (Covey: global magnetic-field/nuclear-Zeeman noise; Fromonteil: technical dephasing vs the claimed *gravitational* decoherence), which the proposals argue is feasible but do not yet decompose.

---

## Classification: **UNDERDETERMINED**

The class claim C2 is classified **UNDERDETERMINED** against Coastline v0.4.

**These proposals are the cleanest available realisation of the cross-probe-mismatch Claim IV anchor and the sharpest instance of the multipartite-but-anti-redundant pole of Claim V — they sharpen the framework even while underdetermined.** Following Coastline v0.4 — which wrote the GR-regime boundary this entry's v0.1 found wanting — the classification is now underdetermined for a **single** reason: the **experimental discriminability** of the GR signature from technical decoherence is argued but not decomposed (D2). The framework-side reason (the GR boundary, v0.1's D1) is **resolved**: v0.4's *Regime of Validity* covers the proposals' weak-field/curvature regime. The classification weighs the present sufficiency of the witness — not the conceptual programme, which these proposals advance.
...
**D1 (framework — GR boundary). ✓ Satisfied by Coastline v0.4 (2026-05-26).** Its *Regime of Validity* brings proper-time relations across differing gravitational potential and (perturbative) curvature within the framework's scope, under the comparison-protocol proviso. The framework-side condition this entry's v0.1 demanded is met; the remaining condition for COMPATIBLE is **D2 alone**.
...
**Relation to the §3.1 open problem (redundant temporal records).** These networks are explicitly *not* a worked exemplar of the framework's principal open problem (synthesis-v0.1 §3.1; Coastline v0.4 Deferred Items): they are multipartite but anti-redundant. A genuinely *redundant* multi-node clock arrangement — many independent nodes each carrying a compatible, separately-readable elapsed-time record (not an entangled collective state) — would, distinctly, bear on Claim II.1 and could be the sought exemplar. This entry sharpens the contrast: entanglement-enhanced networks maximise cross-probe sensitivity precisely by *forgoing* redundancy.

**Feedback to the roadmap (closed loop).** D1 demonstrated that the GR-boundary statement was **load-bearing, not optional** — the framework could not classify the most natural multipartite proper-time experiments (curvature/redshift probes) without it — and **Coastline v0.4 has since written it** (the *Regime of Validity*). This entry's v0.1 is the worked case that motivated the boundary; with v0.4 issued, the feedback loop is closed and D1 is satisfied.

**Relation to CL-2026-006 / CL-2026-007.** All three sit at UNDERDETERMINED, and — after the v0.4 re-evaluation — all three for the *same* reason: the experimental-witness level. CL-2026-008's v0.1 had briefly carried a *second*, framework-internal reason (the unwritten GR boundary); Coastline v0.4 resolved it, so this entry too now rests on the experimental gap alone. It remains the first entry where the multipartite (Claim V) and curvature (External Constraints item 5) commitments are exercised. Declared anchors across the three: mutual information (006), Fisher resolution (007), cross-probe mismatch (008) — the Claim IV pluralism is exercised across the corpus.
...
| Version | Date | Notes |
| --- | --- | --- |
| v0.1 | 2026-05-26 | Initial classification, anchored to Coastline v0.3. Class claim (Covey–Pikovski–Borregaard 2025; Fromonteil et al. 2025) — framed as a class because the two diverge (curvature via 3-node W vs redshift/decoherence via N-atom ensembles) while sharing the entanglement-enhanced/anti-redundant structure. **UNDERDETERMINED** for two reasons: (i) the framework's GR-regime boundary is unwritten (deferred), blocking definitive classification of a curvature/redshift probe — the load-bearing point (D1); (ii) experimental discriminability of the GR signature from technical decoherence (D2). First entry to: apply the resolution/classification anchor distinction (classification anchor = cross-probe mismatch Δω; resolution anchor = QFI/coherence budget); exercise the v0.3 "multipartite ≠ redundant" distinction (these networks are multipartite but anti-redundant, and *do not* claim redundancy — so anti-redundancy is not held against C2); and show the GR-boundary roadmap item is load-bearing, not optional. **Rev. 2026-05-26 (Verifier wording fix, in place — no load-bearing change):** the GR-boundary subsection no longer says Covey is "beyond the Newtonian/weak-field regime"; corrected to "a higher-order post-Newtonian term within weak-field GR, but outside the Coastline's written SR/inertial-worldline scope." The point is the framework's boundary, not a departure from weak-field physics. |
| v0.2 | 2026-05-26 | **Re-anchored v0.3 → Coastline v0.4 and re-evaluated.** Coastline v0.4 wrote the GR-regime boundary (its *Regime of Validity*, extending the framework to the SR-and-weak-field/post-Newtonian regime) that v0.1's **D1** demanded — so the **framework-side gap is now closed** and D1 is marked ✓ satisfied. Classification **remains UNDERDETERMINED**, but now rests on a single reason: the experimental-discriminability condition **D2** (isolating the GR cross-probe signature from technical decoherence). The GR-boundary stress-test subsection, Verdict, Classification statement, D1, and the roadmap-feedback / CL-006-007-relation notes were rewritten accordingly; version self-references v0.3→v0.4. No change to the declared anchors (cross-probe classification / QFI resolution), the multipartite-but-anti-redundant reading, or D2 / D−1. |
