...
- **GR boundary statement.** ⬆ **Elevated to a prerequisite (no longer optional) by CL-2026-008 D1, 2026-05-26.** The framework currently scopes SR proper-time among inertial worldlines; CL-2026-008 shows it cannot definitively classify the most natural multipartite proper-time experiments — Covey's *curvature* residual Δω ∝ d²/R³ and Fromonteil's *redshift* across gravitational potential — until a curved-spacetime / post-Newtonian boundary statement is committed. Add an explicit boundary note bounding or extending Claim II/III to proper-time relations across differing gravitational potential and non-zero curvature.
...
- **CL-2026-008 — opened 2026-05-26** ([`ledger/CL-2026-008-clock-networks-v0.1.md`](../ledger/CL-2026-008-clock-networks-v0.1.md)). Distributed entanglement-enhanced clock-network proposals, framed as a **class claim** (Covey–Pikovski–Borregaard 2025 — curvature via 3-node W-state Δω; Fromonteil et al. 2025 — redshift/decoherence via N-atom ensembles). **UNDERDETERMINED** against Coastline v0.3. First entry to: declare cross-probe mismatch as the **classification anchor** (with QFI/coherence as a separate **resolution anchor**); exercise **multipartite ≠ redundant** (these networks are multipartite but anti-redundant, and do not claim redundancy); and demonstrate the **GR-boundary item is load-bearing** (D1) — the framework cannot definitively classify a curvature/redshift probe until that boundary is written.
