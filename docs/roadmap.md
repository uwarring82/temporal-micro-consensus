...
- **Toy-model functional for temporal redundancy.** ✅ **Drafted; current v0.3 (2026-05-27)** — [`docs/notes/temporal-redundancy-functional-v0.3.md`](notes/temporal-redundancy-functional-v0.3.md) (v0.1, v0.2 frozen). Defines a Riedel-style redundancy functional `R_δ[τ]` on a finite temporal pointer; computes the redundant (product) vs anti-redundant (GHZ-shadow) **analytic poles** (§2), rendering *multipartite ≠ redundant* numerical (`R_{0.10} = 7.1` vs `1`); maps onto the Sorci case (CL-2026-006) and its D1–D3; grounds the resolution/classification-anchor split (above). **v0.3 adds the numerical companion (§8):** the **many-carrier dynamical exemplar** (toolkit `tmc-numerics` Module 3b) — `ε(γ_φ,t)` *derived* from a single-carrier master equation, `R_δ(N, γ_φ t)` curves validated against §2 by the D5 cross-link, **matching the configurational `R_δ` computation of arXiv:2509.17775** on the temporal side. **Residual (still open):** the einselection is *assumed* (§1.1, §8.6), so **Anti-Claim #6 stays open** — a continuous-`τ`, dynamically-einselected exemplar remains the next instrument. Whether any instance counts as *the* worked exemplar is a Coastline-level steward decision.
...
| v0.7 | 2026-05-27 | Recorded **MN v0.3 issued** (`docs/notes/temporal-redundancy-functional-v0.3.md`; v0.2 frozen): the numerical companion **§8** adds the open-temporal-instrument exemplar (toolkit Module 3b) — `ε` *derived* from a single-carrier master equation, `R_δ(N, γ_φ t)` curves, the D5 cross-link to §2, and the structural *redundancy-without-entanglement* point — **matching the configurational `R_δ` computation of arXiv:2509.17775** on the temporal side. §1–§7 + Appendices carried verbatim; §2 analytics byte-identical. Einselection caveat carried verbatim (§8.6) — **Anti-Claim #6 stays open**. Cascade: `index.md` / README tree / `docs/tutorial.md` MN pointers → v0.3; the temporal-redundancy item above updated. **Next Phase-4 (final):** journal-paper integration (Coastline + MN v0.3 + the worked exemplar). |
