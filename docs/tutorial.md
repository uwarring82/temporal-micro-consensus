...
| **Multipartite vs. redundant** | Many carriers can be entangled (anti-redundant) or independent (redundant). Only the latter satisfies Claim II.1. *Now computed:* `R_{0.10} ≈ 7.1` (redundant) vs `1` (anti-redundant) — see the [temporal-redundancy functional note](notes/temporal-redundancy-functional-v0.3.md). |
