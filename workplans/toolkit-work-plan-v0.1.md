...
**Lineage (at plan inception):** Coastline v0.4 (Consensus-Emergence of Classical Proper Time) → Ledger CL-2026-006 v0.4 (Sorci et al. classification, D1 nuisance-decomposition forward target) → MN v0.2 (Temporal-Redundancy Functional R_δ, §2 analytic poles, §6c open-temporal-instrument sketch) → this plan. **Lineage (current):** Coastline v0.4 → Ledger CL-2026-006 **v0.5** (D1 now grounded by Module 3a, classification still UNDERDETERMINED; v0.5 issued 2026-05-27) → MN **v0.3** (numerical companion §8 added, derived `ε`, master-equation `R_δ(N, γ_φ t)` curves; v0.3 issued 2026-05-27) → this plan. The in-text section references below (e.g. "MN v0.2 §2", "MN v0.2 §6c", "CL-2026-006 v0.4 D1") are preserved as the *planning-time* anchors of each commitment; current cross-checks against the v0.3 / v0.5 documents are recorded in their respective version-history entries.
...
- Output: nuisance-budget table decomposing predicted `I(C:M)` (or visibility) loss into per-channel contributions at experimentally relevant parameters. CL-2026-006 D1 names five *physical-channel* contributions — (i) time-dilation unitary entanglement, (ii) motional heating, (iii) motional dephasing, (iv) preparation infidelity, (v) detection-correlated infidelity — which the toolkit's four *computational* layers locate as: (i) the Hamiltonian *signal* (not a nuisance); (ii) and (iii) both the dynamical-Lindblad layer; (iv) the initial-state layer; (v) the read-out layer. The sweep additionally covers drive-phase noise (stochastic-Hamiltonian layer) and mode-mixing — channels in CL-2026-006's wider discriminability gap but outside D1's formal five — so the budget is a *superset* of the D1 decomposition, not a one-to-one image of it.
...
**On Discriminant D1 (necessary, not sufficient).** D1 is one of three conditions (D1–D3) for upgrading CL-2026-006 from UNDERDETERMINED to COMPATIBLE; D2 (null test) and D3 (a *measured* `r`-scan) are experimental and lie outside the toolkit. The toolkit produces the *theoretical* per-channel decomposition D1 names — it does not, by itself, upgrade the verdict; Phase 4 revisits the verdict under the new evidence rather than pre-deciding it. CL-2026-006 also names **CL-2026-005 (the Colla–Breuer–Gasbarri / CBG pipeline)** as the methodological infrastructure for this very decomposition. Q5 (§8) resolves the relation: Module 3a is reimplemented in QuTiP (per D3), with CBG retained only as an optional non-Markovian validation backend and a methodological template — not a host or dependency, because CBG computes the coherent effective Hamiltonian `K(t)` (non-Markovian), not the Markovian-Lindblad nuisance/mutual-information budget D1 requires. *Registered flag (for CL-2026-006 v0.5, Phase 4):* the entry's "produces precisely the nuisance-channel decomposition required by D1" overstates this scope mismatch and should be softened when v0.5 is drafted.
...
## §8 — Architectural questions (registered at v0.1; Q3 and Q5 resolved)
...
- *Q5 (resolved at v0.1 rev. (c)).* Relation to CL-2026-005 (the Colla–Breuer–Gasbarri / **CBG** non-Markovian TCL effective-Hamiltonian pipeline, `oqs-cbg-pipeline`), named by CL-2026-006 as the D1-decomposition infrastructure. **Resolved: reimplement in QuTiP (per D3); CBG is not a host or hard dependency.** CBG computes the coherent effective Hamiltonian `K(t)` in the non-Markovian regime, not the Markovian-Lindblad mutual-information nuisance budget D1 requires (scope mismatch); hosting Module 3a there would also import CBG's Council-clearance discipline and its *permanent* trapped-ion stewardship-conflict flag (CL-2026-005 Entry 6) under a different anchor chain. CBG is retained as (a) an *optional* non-Markovian validation backend — its HEOM/pseudomode reference solvers, for later cross-checks of the Markovian approximation (Phase 3+/v0.2, not a Phase 1 dependency); and (b) a methodological template — its benchmark-card / frozen-parameter / cause-labelled failure-envelope / validity-envelope discipline, adopted for the toolkit's own results archival. See §6.
...
- **v0.1 rev.** (2026-05-27, post-review): Folded a three-pass review (steward + IDE reviewer + Claude), nine findings, tightened in place. **§3/§5:** `R_δ` post-processor now built on an explicit `TemporalRecordEnsemble` read-out abstraction (pointer `T`, `P(T)`, per-carrier read-out maps, deficit `δ`, undefined case), distinguished from quantum `I(C:M)`; cross-anchor tests rewritten as explicit invariants encoding the resolution/classification split (guarding the CL-2026-007 v0.1 failure mode of reading `F[τ]` as a classifier). **§1/§3:** nuisance model split into four layers (initial-state / dynamical-Lindblad / stochastic-Hamiltonian / read-out), located against CL-2026-006 D1's five contributions (a different taxonomy — refined in rev. (b)); dropped the stray squeezing knob `r` from the redundant-pole observable (`R_δ(N, ε)`, `ε` dynamically derived); Module 3a `R_δ` cross-check rewritten as a directional N=2 transition (not an `R_δ(r)` curve). **§1/§6:** "satisfies D1" softened to "produces the D1 decomposition" (D1 = 1 of 3 conditions; D2/D3 experimental); CL-2026-005/CBG relation registered (Q5); "first worked exemplar" → "candidate exemplar within the assumed regime". **§7:** publication staging reframed as a proposal pending the roadmap's open external-circulation decision. **§8:** Q3 (result archival) resolved; Q5 (CBG) added. No locked decision (D1–D6) changed.
- **v0.1 rev. (b)** (2026-05-27): §3 Module 3a — corrected the D1 mapping (Guardian review). CL-2026-006 D1 enumerates five *physical-channel* contributions (i)–(v): signal, heating, dephasing, preparation infidelity, detection infidelity. The toolkit's four *computational* layers are a distinct taxonomy — heating and dephasing share the dynamical-Lindblad layer, the time-dilation signal sits in the Hamiltonian (not a nuisance), and the sweep additionally covers drive-phase noise and mode-mixing (CL-2026-006's wider discriminability gap, outside D1's formal five). The rev. (a) phrasing "four layers map one-to-one onto D1(i)–(v)" was inaccurate on both counts; the budget is a superset of the D1 decomposition. *(Note: the Guardian's suggested fix placed drive-phase noise inside D1(i)–(v); verified against CL-2026-006 line 120, which does not — drive-phase noise is gap item 4, not a D1 contribution.)*
- **v0.1 rev. (c)** (2026-05-27): Resolved **Q5** (CBG reconciliation) after reading CL-2026-005 v0.4 + the `oqs-cbg-pipeline` validity envelope. CBG is the Colla–Breuer–Gasbarri non-Markovian TCL effective-Hamiltonian pipeline; it computes the coherent `K(t)`, not the Markovian-Lindblad nuisance/MI budget D1 requires, and carries a permanent trapped-ion stewardship-conflict flag (CL-2026-005 Entry 6). **Resolution:** Module 3a reimplemented in QuTiP (per D3); CBG retained as an *optional* non-Markovian validation backend + methodological template, not a host/dependency (§6, §8). Registered a flag to soften CL-2026-006's "produces precisely the nuisance-channel decomposition" wording at v0.5 (Phase 4). No locked decision (D1–D6) changed.
- **v0.1 rev. (d)** (2026-05-28): **Registered toolkit-v0.2 hygiene item** (Guardian, post-HV-DRAFT-001 6c). The current local toolchain splits across two Python installations — homebrew Python 3.13 (where `qutip 5.2.3`, `tmc_numerics`, `matplotlib`, `numpy`, `pytest` actually live) and an older anaconda3 Python 3.9 (where the default `pip` / `pytest` / `jupyter` binaries point). The HV-DRAFT-001 6c notebook execution required (a) registering the homebrew 3.13 as a Jupyter kernel (`tmc-python313`) and (b) `PYTHONPATH=numerics/src` to make `tmc_numerics` importable without an editable install (PEP-668 blocks editable installs into the homebrew env). Functional, but brittle for future contributors. **Backlog for toolkit v0.2:** single virtual-environment setup with pinned `requirements.txt` (or `requirements-lock.txt` per the existing §4 pinning policy) and a proper editable install (`pip install -e .` inside the venv), so first-time-clone reproducibility is one command. Not a blocker for toolkit v0.1 release; recorded so it doesn't fall off the radar. No locked decision (D1–D6) changed.
- **v0.1 rev. (e)** (2026-05-29): **Lineage-block refresh** (HV-DRAFT-001 Step-3 D-queue, agent pass). The header *Lineage:* line was the only place the work plan stated its current downstream artefacts — and it had drifted: CL-2026-006 was at v0.4 at plan inception and is now at v0.5 (D1 grounded by Module 3a), and MN was at v0.2 at plan inception and is now at v0.3 (numerical companion §8). Updated to record both the planning-time and current anchors, so the §3 / §5 / §6 in-text "MN v0.2 §2" and "CL-2026-006 v0.4 D1" cross-references are preserved as planning-time pointers (the §-numbering at v0.2 / v0.4 differs from v0.3 / v0.5 and a global rewrite would falsify the planning audit trail). Cross-checks against the v0.3 / v0.5 documents are recorded in those documents' own version histories. No locked decision (D1–D6) changed; no scope change; no §6 staged-feed-forward change.
